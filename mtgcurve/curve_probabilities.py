...
def calculate_cmc_probs(num_cards_in_library, mana_producers, num_lands, max_turns=1, max_mana=7):
    num_opening_hands = 0
    ...
    def thread_calc_prob_table(hand_state, hand_prob):
        ...
        def calculate_subtree_probs(state, initial_prob):
            ...
            if mana < 0:
                print(state)
                exit()
            if state.turn_number == 1 and mana >= max_mana:
                print(state)
                exit()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for num_lands_in_hand, mana_producers_in_hand, num_other_cards_in_hand, initial_prob in starting_hand_generator():
            ...
            sub_prob_table = thread_calc_prob_table(state, initial_prob)
