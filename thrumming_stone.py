...
def thrumming_stone(num_cards_in_library=80, num_apostles_in_library=33):
    probs = prob_apostles(num_cards_in_library, num_apostles_in_library)
    table_data = [
        ['Apostles in Library', 'Probability'],
        *[[i, f'{p:.9f}'] for i, p in enumerate(probs)],
    ]

    table = terminaltables.AsciiTable(table_data)

    print('Arguments:')
    print('\tLibrary Size:', num_cards_in_library)
    print('\tNum Same Card:', num_apostles_in_library)
    print(table.table)


if __name__ == '__main__':
    fire.Fire(thrumming_stone)
